use super::{keystore::KeyStore, constants::KIND_REGISTRATION};
use super::nostr_utils::initialize_nostr;
use musig2::{
    secp256k1::{PublicKey, SecretKey},
    FirstRound, KeyAggContext, PartialSignature, PubNonce, SecondRound,
};
use nostr::{Event, Filter, Keys as NostrKeys, Keys, Kind, Tag, PublicKey as NostrPublicKey, Timestamp, EventId};
...
impl Signer {
    ...
    pub async fn register_with_coordinator(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let event = Event::new(
            ,
            self.coordinator_nostr_pubkey,
            Timestamp::now(),
            Kind::EncryptedDirectMessage,
            vec![Tag::public_key(self.coordinator_nostr_pubkey)],
            registration_msg,
            &self.nostr_keys,
        );
    ...
    pub async fn start_first_round(&mut self) -> Result<PubNonce, Box<dyn std::error::Error>> {
        ...
        let event = Event::new(
            ...
            format!(
                "pub_nonce: session_id={}, nonce={}",
                self.session_id,
                serde_json::to_string(&pub_nonce)?
            ),
            &self.nostr_keys,
        )?;
        client.send_event(event).await?;
    ...
    pub async fn wait_for_nonces_and_signal(
        &mut self,
    ) -> Result<Vec<PubNonce>, Box<dyn std::error::Error>> {
        let mut client = self.client.lock().await;
    ...
    pub async fn handle_second_round(
        &mut self,
        all_nonces: Vec<PubNonce>,
        ...
        let partial_sig = first_round
            .sign(&self.secret_key, b"message_to_sign")
            .unwrap();
        let mut client = self.client.lock().await;
        let event = Event::new(
            Kind::EncryptedDirectMessage,
            vec![Tag::PublicKey(self.coordinator_nostr_pubkey)],
            format!(
                "partial_sig: session_id={}, sig={}",
                self.session_id,
                serde_json::to_string(&partial_sig)?
            ),
            &self.nostr_keys,
        )?;
        client.send_event(event).await?;
